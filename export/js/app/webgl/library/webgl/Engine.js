...
window.gameInitFunctions["gameInit2"].push(function () {
	...
	keyboard.register({
		...
		down: function () {
			let mipChange = true;
			if(window.engine.RENDER_MODE != "debug_downsample") {
				window.engine.DEBUG_MIP_LEVEL = 0;
				mipChange = false;
			}
			window.engine.programMode("debug_downsample");

			if(mipChange){
				window.engine.DEBUG_MIP_LEVEL++;

				if(5 < window.engine.DEBUG_MIP_LEVEL) window.engine.DEBUG_MIP_LEVEL = 0;
			}
		}
	});
	keyboard.register({
		...
		down: function () {
			let mipChange = true;
			if(window.engine.RENDER_MODE != "debug_upsample") {
				window.engine.DEBUG_MIP_LEVEL = 4;
				mipChange = false;
			}
			...
			if(mipChange){
				...
				if(window.engine.DEBUG_MIP_LEVEL < 0) window.engine.DEBUG_MIP_LEVEL = 4;
			}
		}
	});
